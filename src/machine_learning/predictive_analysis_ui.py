...
def predict_sale_price(X_live, sale_price_features, sale_price_pipeline, sale_price_labels_map):

    # from live data, subset features related to this pipeline
    X_live_tenure = X_live.filter(sale_price_features)

    # predict
    sale_price_prediction = sale_price_pipeline.predict(X_live_tenure)
    sale_price_prediction_proba = sale_price_pipeline.predict_proba(X_live_tenure)
    # st.write(sale_price_prediction_proba)

    # create a logic to display the results
    proba = sale_price_prediction_proba[0, sale_price_prediction][0]*100
    sale_price_levels = sale_price_labels_map[sale_price_prediction[0]]
